...
class Model(object):
    ...
    pcolorgivendist = {'red': [0.7, 0.3, 0.1, 0],
    ...
    def sample(self, distance):
        ...
        """
        dist = []
        for each_color in self.pcolorgivendist:
            farthest = len(self.pcolorgivendist[each_color]) - 1
            if distance > farthest:
                d = farthest
            else:
                d = distance
            for i in range(int(self.pcolorgivendist[each_color][d] * 100)):
                dist.append(each_color)
        return random.choice(dist)

    def get_pcolorgivendist(self, color, dist):
        """
        ...
        """
        farthest = len(self.pcolorgivendist[color]) - 1
        if dist <= farthest:
            return self.pcolorgivendist[color][dist]
        else:
            return self.pcolorgivendist[color][farthest]
