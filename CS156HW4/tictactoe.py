...
class GameState(object):
    ...
    def is_win(self, player):
        ...
        We only check the lines that may have been affected by the last move. Good!
        ...
        if self.size == self.board[row].count(player):
            return True
        # check the diagonal - if the last move was on a diagonal: 45 degrees
        if row == col:  # last move was on first diagonal, Good
            if self.size == [self.board[i][i] for i in range(self.size)].count(player):
                return True
        if row == self.size - 1 - col:  # last move was on second diagonal: 135 degrees
            if self.size == [self.board[i][self.size - 1 - i] for i in range(self.size)].count(player):
                return True
        # check the column affected by the last move
        if self.size == [self.board[i][col] for i in range(self.size)].count(player):
                return True
        return False
    ...
    def is_tie(self):
        ...
        :return: boolean
        """
        return self.moves == self.size ** 2  # ** is power
    ...
    def possible_moves(self):
        ...
        """
        # There is an order of exploring successors, so starting from up left corner is no
        # the same as starting from the bottom left corner. It is not symmetric.
...
class Game(object):
    ...
    search (string): the adversarial search algorithm to be used by our
        AI agent: "rand", "minimax", "alphabeta" or "abdl"
    depth (int):  depth limit for use with the abdl algorithm only
    state (Gamestate): object to keep track of underlying state of the game
    """
